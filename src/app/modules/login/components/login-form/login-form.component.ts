...
import {InvalidCredentialsComponent} from "../../../../shared/invalid-credentials/invalid-credentials.component";
import {LoginModelRequest} from "../../../../models";
  ...
  onSubmit() {
    ...
    if (this.loginForm.valid) {
      this.loginService.login(object).subscribe({
        next: (res) => {
          if (res!=null) {
              let factoryDTO = JSON.parse(JSON.stringify(res)).factoryDTO
              let id = JSON.parse(JSON.stringify(res)).id;
              if(factoryDTO != null){
                this.loginService.setUtenteSession(res?.username, res?.userInfoDTO, res?.usertype, id, factoryDTO?.id);
              }
              else{
                  this.loginService.setUtenteSession(res?.username, res?.userInfoDTO, res?.usertype, id);
              }
            ...
            switch (res.usertype.toLowerCase()) {
              case "superadmin":
                ...
                break;
              case "admin":
                this.router.navigate(['/gestionale/prodotti']);
                break;
              case "user":
            ...
            this.log="Credenziali errate";
          }

          this.loaderSpinnerService.hide()
          /*if (res==null) {
            this.loaderSpinnerService.hide()
            this.dialog.open(InvalidCredentialsComponent, {
              width: '660px',
              height: '300px',
              disableClose: true,
            });
