...
import {SignupModelRequest, UserinfoModel} from "../../../models";
...
@Component({
  ...
  templateUrl: '../modifica-profilo/modifica-profilo.component.html',
  styleUrls: ['../modifica-profilo/modifica-profilo.component.scss'],
  imports: [
    CommonModule,
    RouterModule,
    MatButtonModule,
...
export default class UpdateProfileComponent{
  ...
  ngOnInit() {
    ...
    this.utentiService.readUser(this.id).subscribe(
        result => {
          ...
          else { this.usertype="superadmin"; this.isAdmin = false; }
  ...
  update(){
    ...
    let updateRequest : UpdateModelRequest = {
      ...
      usertype:this.usertype.toUpperCase() as string,
      ...
      factory_city:this.factory_city,
      factory_province:this.factory_province,
        ...
        next: value  => {
          ...
          if (this.log == "") {
            this.signupService.update(updateRequest).subscribe({
              next: value => {
                if (updateRequest.image!=null) this.hasImage=true;
                ...
                dialogRef.afterClosed().subscribe(() => {
                  let userInfoDTO : UserinfoModel = {address: updateRequest.address, city: updateRequest.city, date: updateRequest.birthday, genderType: updateRequest.genre, id: updateRequest.id, lastname: updateRequest.lastname, name: updateRequest.firstname, nation: updateRequest.nation, province: updateRequest.province}
                  this.loginService.setUtenteSession(updateRequest.email, userInfoDTO, updateRequest.usertype, updateRequest.id);
                  this.goToProfile();
                })
              },
              error: error => {
