...
export class LoginService {
  ...
  setUtenteSession(username: string, userInfo : object,  usertype: string, id?:number ,idFactory?: number): void {
    if(usertype.toLowerCase() == 'admin' || 'superadmin'){
      const userSession = { username, usertype, idFactory, id, userInfo};
      localStorage.setItem('userSession',JSON.stringify(userSession));
      this.updateNominativoUtente.next(userSession);
    }
    else if((usertype.toLowerCase() == 'user')){
      const userSession = { username, usertype, id}
      this.updateNominativoUtente.next(userSession);
    }
    //else{console.log("Chiaramente stoogaz")}
